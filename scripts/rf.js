// 1- Add the back button
// 2- done!  recalculate top 10 lsp features for med and symp. 
// 3- done! highligh med and symp in different colors 

// 4- ulms features for later. 
// 5- sentence based classification for later. 
// 
...
function readData(){
	...
	d3.json("./data/RF_Classifier/model/rf_model.json", function(jsondata){
		...
		var list = chart_svg.append("g").append("rect")
						...
						.attr("width", list_width)
						.attr("height", list_height)
						.attr("fill", "#6cc04a")  // #71a087
						.style("fill-opacity",0.3)
						.style("stroke","#6cc04a");  // #00b3e3
		...
		var bars = chart_svg.append("g").attr("class","bars").selectAll(".bars").data(jsondata).enter()
						...
						.attr("y", function(d,i){return list_y + d.count*clearance + 1.05*clearance })
